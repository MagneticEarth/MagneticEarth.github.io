...
- The JS/CSS that Panel/Bokeh figures depend on is vendored into `assets/vendor/` rather than loaded from unpkg.com, which was returning 500s and 60s timeouts for some of these files and blocking page render. Panel exports (e.g. `pages/figs/hvplot_mag_obs_map.html`) point at these local copies, so **if you regenerate such a figure, rewrite its `unpkg.com` URLs to `/assets/vendor/...` again** — otherwise the page will go back to hanging on unpkg.
